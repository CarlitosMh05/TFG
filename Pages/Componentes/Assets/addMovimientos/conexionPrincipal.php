...
$es_ingreso = isset($_POST['es_ingreso']) ? intval($_POST['es_ingreso']) : 0;
...
try {
    ...
    $stmt = $conn->prepare("SELECT id FROM conceptos WHERE user_id=? AND nombre=? AND es_ingreso=? LIMIT 1");
    $stmt->bind_param('isi', $userId, $concepto, $es_ingreso);
    $stmt->execute();
    $stmt->bind_result($conceptoId);
    if (!$stmt->fetch()) 
    {
        $ins = $conn->prepare("INSERT INTO conceptos(user_id,nombre,es_ingreso) VALUES(?,?,?)");
        $ins->bind_param('isi', $userId, $concepto, $es_ingreso);
        $ins->execute();
